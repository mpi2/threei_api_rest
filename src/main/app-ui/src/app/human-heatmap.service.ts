...
import {HumanCellFilter} from "./human-cell-heatmap/human-cell-filter";


@Injectable({
  providedIn: 'root'
})
export class HumanHeatmapService {
  ...
  getCellHeatmapResponse(filter: HumanCellFilter):
    ...
    const urlstring = this.restBaseUrl + '/human_cell_heatmap' + filterString;
  ...
  getFilterString(filter: HumanCellFilter) {
    let filterQuery = '';
    if (filter) {
