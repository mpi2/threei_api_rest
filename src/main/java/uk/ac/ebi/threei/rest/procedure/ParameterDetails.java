...
public class ParameterDetails {
	...
	/**
	 * used by procedure page to only display data for each header type and not
	 * multiples per header
	 * 
	 * @return
	 */
	public List<Result> getMostSignificantResults() {
		// some parameters have multiple entries for the same sex zyg combination which
		// we just need the most significant one
		List<Result> allResults = new ArrayList<>();
		for (SexType key : resultsBySex.keySet()) {
			...
			for (Result result : sexResults) {
				...
				if (zygositySet.containsKey(result.getHeaderKey())) {
					...
					// System.out.println("headerkey added"+result.getHeaderKey() +" sign
					// "+result.getSignificant());
					zygositySet.put(result.getHeaderKey(), result);
				}

			}
			allResults.addAll(zygositySet.values());// add all the values for the sex that have had duplicates with
													// different sig values removed to be just the highest significant
													// one.
		}
		return allResults;
	}

	private Result getOnlyMostSignificantResultForHeader(HashMap<String, Result> zygositySet, Result result) {
		...
		if (SignificanceType.getRankFromSignificanceName(resSig.name()) >= SignificanceType
				.getRankFromSignificanceName(prevSig.name())) {
			return result;
		} else {
