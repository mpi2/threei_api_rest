...
import uk.ac.ebi.threei.rest.repositories.human.HumanCellParameterRepository;

@Service
public class HumanCellHeatmapService {

	@Autowired
	HumanCellHeatmapRowsRepository humanCellHeatmapRowsRepository;
	@Autowired
	HumanCellParameterRepository humanCellRepo;
	@Autowired
	HumanGeneService geneService;
...
//System.out.println("row="+row);
		...
		ArrayList<String> cellHeaders = new ArrayList<>(
				Arrays.asList(HumanCellHeatmapRowsRepository.cellDisplayHeaderOrder));
...
//				}
	   		...
	   		List<CellHeatmapRow> rows=new ArrayList<>();
	   		 if(sort!=null) {
	   			 ...
	   			 rows = humanCellHeatmapRowsRepository.findAll(example, sort);
	   		 }
	   		 else {
	   			 rows= humanCellHeatmapRowsRepository.findAll(example);
	   		 }
			if(filter.getKeyword()!=null) {
...
//	            .withIgnoreCase();  
		...
		exampleRow.setGene("Zranb1");
		 Example<CellHeatmapRow> example = Example.of(exampleRow, exampleMatcher);
		 System.out.println("example="+example);
		 Optional<CellHeatmapRow> singleRow = humanCellHeatmapRowsRepository.findOne(example);
...
//		    setRowCount((int) animals.getTotalElements());
	...
	public Types cellTypes() {
		if (uniqueCellTypes == null) {
			...
			List<CellParameter> dataList = humanCellRepo.findAll();
